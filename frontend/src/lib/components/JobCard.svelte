<script>
  ...
  import { getFilePath, API_BASE_URL } from '$lib/api';
  ...
  async function loadFilePaths() {
    ...
    try {
      ...
      if (job.video_path) {
        const response = await getFilePath('video', job.job_id);
        videoUrl = API_BASE_URL + response.path;
      }
      
      // Load audio path if available
      if (job.audio_path) {
        const response = await getFilePath('audio', job.job_id);
        audioUrl = API_BASE_URL + response.path;
      }
      
      // Load SRT path if available
      if (job.srt_path) {
        const response = await getFilePath('srt', job.job_id);
        srtUrl = API_BASE_URL + response.path;
      }
    } catch (error) {
